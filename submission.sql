--Create Order table statements
CREATE TABLE Customer (email CHAR(20) PRIMARY KEY, CName CHAR(20), address CHAR(50));
CREATE TABLE Employee (employeeID int PRIMARY KEY, EName CHAR(20));
CREATE TABLE EmployeeParking (age int, parkingSpot CHAR(3), experienceLevel CHAR(10),PRIMARY KEY (age, parkingSpot, experienceLevel));
CREATE TABLE Truck (parkingSpot CHAR(3), truckNumber int, PRIMARY KEY (parkingSpot));
CREATE TABLE Driver (age int, experienceLevel CHAR(10), employeeID int, FOREIGN KEY (employeeID) REFERENCES Employee ON DELETE CASCADE, PRIMARY KEY (age, experienceLevel));
CREATE TABLE Packer (employeeID int, basketNum int, FOREIGN KEY (employeeID) REFERENCES Employee ON DELETE CASCADE, PRIMARY KEY (employeeID));
CREATE TABLE GoesTo (employeeID int, email CHAR(20), PRIMARY KEY (employeeID, email), FOREIGN KEY (employeeID) REFERENCES Employee ON DELETE CASCADE, FOREIGN KEY (email) REFERENCES Customer ON DELETE CASCADE);
CREATE TABLE PlaceOrder (orderID int, email CHAR(20), PRIMARY KEY (orderID, email), FOREIGN KEY (email) REFERENCES Customer ON DELETE CASCADE);
CREATE TABLE Package (psize CHAR(10) PRIMARY KEY, stock int);
CREATE TABLE Requires (psize CHAR(10), orderID int, email CHAR(20), PRIMARY KEY (psize, orderID, email), FOREIGN KEY (psize) REFERENCES Package, FOREIGN KEY (orderID, email) REFERENCES PlaceOrder ON DELETE CASCADE);
CREATE TABLE WorksOn (orderID int, email CHAR(20), employeeID int, PRIMARY KEY (orderID, email, employeeID), FOREIGN KEY (employeeID) REFERENCES Employee ON DELETE CASCADE,FOREIGN KEY (orderID,email) REFERENCES PlaceOrder ON DELETE CASCADE);

--Create Product table statements
CREATE TABLE Category (title CHAR(20) PRIMARY KEY, location CHAR(3));
CREATE TABLE EmployeeProduct(employeeID int, productID int PRIMARY KEY, FOREIGN KEY (employeeID) REFERENCES Packer ON DELETE CASCADE);
CREATE TABLE Brand (companyName CHAR(30) PRIMARY KEY);
CREATE TABLE Product (productName CHAR(80), companyName CHAR(30) NOT NULL, employeeID int, PRIMARY KEY (productName, companyName, employeeID),FOREIGN KEY (employeeID) REFERENCES Packer ON DELETE CASCADE,FOREIGN KEY (companyName) REFERENCES Brand);
CREATE TABLE ProductStock (productName CHAR(80), stock int, companyName CHAR(30), PRIMARY KEY (productName, companyName), FOREIGN KEY (companyName) REFERENCES Brand);
CREATE TABLE IsPartOf (productID int, title CHAR(20), PRIMARY KEY(productID, title), FOREIGN KEY (productID) REFERENCES EmployeeProduct, FOREIGN KEY (title) REFERENCES Category);
